...
output_folder = '26037BC_results/cellpose_masking/'
...
if __name__ == '__main__':
    ...
    # in this example indices 0 and 1 are combined for the cell/ cytoplasmic mask, i dont want a nuclear mask 
    imgs_cp = [
        np.stack(
            (
                img[1],                      # just channel 1
                np.zeros_like(img[0]),
                np.zeros_like(img[0])
            ),
            axis=0
    ...
    # ---------------- apply cellpose ----------------
    masks, flows, styles = apply_cellpose(imgs_cp, niter=2000, big_images=True,cellprob_threshold=-2.0, flow_threshold=0.8, diameter=25)
    # check the masks with visualisation, else you can skip this step
    visualise_cell_pose(imgs_cp[:5], masks, flows, big_images=True)
